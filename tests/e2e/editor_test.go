...
func TestCommandPaletteHelpOrientsThenNavigates(t *testing.T) {
	...
	h.assertNotContains("Open File")

	for _, r := range "Open File" {
		h.pressRune(r)
	}
	h.assertContains("Open File")
	cmd, ok := h.reg.Get("file.open")
	if !ok {
		t.Fatal("file.open command is not registered")
	}
	if cmd.Shortcut == "" {
		t.Fatal("file.open should have a derived shortcut")
	}
	h.assertContains(cmd.Shortcut)

	h.pressKey(tcell.KeyEnter, tcell.ModNone)
	if len(h.app.Root.Overlays) == 0 {
		t.Fatal("executing Open File from help should open its dialog")
	}
}
