...
func TestAutocompletePopup(t *testing.T) {
	h := newTestHarness(t, 80, 24)
	defer h.stop()

	h.exec("editor.autocomplete")
	if h.app.editorGroup.Autocomplete == nil {
		t.Fatal("expected autocomplete to be active")
	}
	h.redraw()

	h.assertContains("fmt.Println")
	h.assertContains("fmt.Printf")

	// Navigate down
	h.pressKey(tcell.KeyDown, tcell.ModNone)
	if h.app.editorGroup.Autocomplete.Selected != 1 {
		t.Errorf("expected Selected 1 after Down, got %d", h.app.editorGroup.Autocomplete.Selected)
	}
	...
	h.redraw()
	h.assertNotContains("fmt.Println")
}
...
func TestAutocompleteSelect(t *testing.T) {
	...
	// First item is "fmt.Println" — should be inserted
	line := h.app.editorGroup.Editor.Buf.Lines[0]
	if line != "fmt.Println" {
		t.Errorf("expected buffer to contain 'fmt.Println', got %q", line)
	}
}
...
func TestAutocompleteTabCompletes(t *testing.T) {
	...
	h.exec("editor.autocomplete")
	h.pressKey(tcell.KeyDown, tcell.ModNone) // select second item (fmt.Printf)
	...
	line := h.app.editorGroup.Editor.Buf.Lines[0]
	if line != "fmt.Printf" {
		t.Errorf("expected buffer to contain 'fmt.Printf', got %q", line)
	}
}

func TestAutocompleteTypingPassthrough(t *testing.T) {
	h := newTestHarness(t, 80, 24)
	defer h.stop()

	h.exec("editor.autocomplete")

	// Regular keypresses should pass through to the editor
	h.pressRune('h')
	h.pressRune('i')

	line := h.app.editorGroup.Editor.Buf.Lines[0]
	if line != "hi" {
		t.Errorf("expected 'hi' in buffer, got %q", line)
	}

	// Autocomplete should still be active
	if h.app.editorGroup.Autocomplete == nil {
		t.Error("expected autocomplete to still be active while typing")
	}
}
