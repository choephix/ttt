...
// italic.
func drawTabLabel(surface Surface, x, innerLeft, innerRight int, label string, dirty bool, style term.Style, italic bool) {
...
func (t *TabBarWidget) Render(surface Surface) {
	...
	for i, s := range spans {
		...
		if s.active {
			...
			drawTabLabel(surface, sx+1, innerLeft, innerRight, s.label, dirty, term.StyleActiveTab, t.Tabs[i].Preview)
			if ex-1 >= innerLeft && ex-1 < innerRight {
				surface.SetCell(ex-1, 1, term.Cell{Ch: b.Vertical, Style: bs})
			}
		} else {
			drawTabLabel(surface, sx, innerLeft, innerRight, s.label, dirty, term.StyleInactiveTab, t.Tabs[i].Preview)
		}
	}
